...
相比原有的 `fineweb-med.py`，新版本 `fineweb-med-new.py` 包含以下改进：

### 1. 完整的质量过滤管道
- **URL过滤**: 移除低质量或不需要的URL
- **语言过滤**: 只保留英文内容
- **重复内容过滤**: 使用Gopher重复过滤器
- **质量过滤**: C4和FineWeb质量过滤器
- **医疗内容过滤**: 只保留包含医疗关键词的文档
- **长度过滤**: 确保文档至少200个单词

### 2. 去重处理
- 使用Minhash算法进行近似去重
- 多阶段去重流程：签名生成 → 分桶 → 聚类 → 过滤
- 针对医疗内容的参数优化
