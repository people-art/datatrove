...
# FineWeb-Data HuggingFace Upload Script
...
#   ./publish_to_hf.sh --domain education # Upload datasets for specific domain
...
REPO_BASE_NAME="fineweb-data"  # Default repository base name
DOMAIN_FILTER=""  # Optional domain filter
OUTPUT_BUCKET="${OUTPUT_BUCKET:-fineweb-data}"  # Configurable output bucket

while [[ $# -gt 0 ]]; do
    case $1 in
        --help|-h)
            echo "FineWeb-Data HuggingFace Upload Script"
            echo ""
            echo "Usage:"
            ...
            echo "  $0 --name my-dataset          # Set custom dataset name (default: fineweb-data)"
            echo "  $0 --domain education         # Filter by domain (optional)"
            echo "  $0 --help                     # Show this help"
            echo ""
            echo "The script automatically scans s3://fineweb-data/base_processing/output/"
        ...
        *)
            ...
            echo "Usage: $0 [--latest|--all|--dump DUMP_ID|--name NAME|--domain DOMAIN|--help]"
            exit 1
            ;;
    esac
done
...
# Will be updated based on domain filter
...
try:
    s3 = boto3.client('s3')
    response = s3.list_objects_v2(Bucket='fineweb-data', Prefix='base_processing/output/')
    dumps = []
    ...
    print('CC-MAIN-2024-22\nCC-MAIN-2024-18\nCC-MAIN-2024-10', file=__import__('sys').stderr)
" 2>/dev/null)
    fi
...
echo "🚀 Starting FineWeb-Data dataset upload to HuggingFace Hub"

# Set BASE_DIR based on domain filter and output bucket
if [ -n "$DOMAIN_FILTER" ]; then
    DOMAIN_SUFFIX=$(echo "$DOMAIN_FILTER" | tr '[:upper:]' '[:lower:]' | tr ' ' '-')
    BASE_DIR="s3://${OUTPUT_BUCKET}/base_processing/${DOMAIN_SUFFIX}/output"
else
    echo "❌ ERROR: --domain parameter is required for proper data isolation"
    echo "   Use: ./publish_to_hf.sh --domain 'your-domain' [other options]"
    exit 1
fi
...
if [ $? -ne 0 ]; then
    ...
    echo "   For FineWeb-Data, run: python finewebdata.py --domain <domain> --year <year>"
    exit 1
fi
...
for dump_id in $selected_dumps; do
    ...
    # For domain-specific datasets, include domain in repository name
    if [ -n "$DOMAIN_FILTER" ]; then
        DOMAIN_SUFFIX=$(echo "$DOMAIN_FILTER" | tr '[:upper:]' '[:lower:]' | tr ' ' '-')
        CURRENT_REPO_NAME="${HF_USERNAME}/${REPO_BASE_NAME}-${DOMAIN_SUFFIX}"
        echo "🎯 Domain-specific repository: $CURRENT_REPO_NAME"
    else
    ...
    # Check if upload_to_huggingface.py exists
    UPLOADER="../tools/upload_to_huggingface.py"
    if [ ! -f "$UPLOADER" ]; then
        UPLOADER="../fineweb-med/upload_to_huggingface.py"  # Fallback to fineweb-med
        if [ ! -f "$UPLOADER" ]; then
            echo "❌ ERROR: upload_to_huggingface.py not found!"
            echo "   Expected locations:"
            echo "     - ../tools/upload_to_huggingface.py"
            echo "     - ../fineweb-med/upload_to_huggingface.py"
            exit 1
        fi
    fi
...
if [ -n "$DOMAIN_FILTER" ]; then
    DOMAIN_SUFFIX=$(echo "$DOMAIN_FILTER" | tr '[:upper:]' '[:lower:]' | tr ' ' '-')
    REPO_URL="https://huggingface.co/datasets/${HF_USERNAME}/${REPO_BASE_NAME}-${DOMAIN_SUFFIX}"
    echo "🌐 View your domain-specific dataset at: $REPO_URL"
elif [ "$dump_count" -gt 1 ]; then
    echo "🌐 Uploaded datasets:"
    for dump_id in $selected_dumps; do
        REPO_SUFFIX=$(echo "$dump_id" | tr '[:upper:]' '[:lower:]' | sed 's/cc-main-/cc/')
        REPO_URL="https://huggingface.co/datasets/${HF_USERNAME}/${REPO_BASE_NAME}-${REPO_SUFFIX}"
        echo "   📦 $dump_id → $REPO_URL"
    done
else
